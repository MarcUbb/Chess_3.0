import AI_s.ai_helper_functions
...
def ai_function(board, team):
    ...
    for i in board.fields:
        for f in i:
            if f.active == True:
                king_moves.append(f)
    AI_s.ai_helper_functions.reset_markers(board)

    for i in board.fields:                                              #activates all move_patterns for enemy team
        for f in i:
            if f.figure_team != team:
                AI_s.ai_helper_functions.mark_move_pattern(f, board)
...
#############################################################
    ...
    while i < len(movable_figures):
        moves.append([])
        move_counter = 0
        AI_s.ai_helper_functions.mark_move_pattern(movable_figures[i], board)
    ...
    while i < len(moves):                                                   # gives every move that checks the ai the value -1
        ...
        while j < len(moves[i]):
            ...
            test_board.fields[moves[i][j][0].pos_y][moves[i][j][0].pos_x].move_pattern = AI_s.ai_helper_functions.pattern_generator(moves[i][j][0].figure_type, moves[i][j][0].figure_team)  # also the pattern has to be raplaced since we dont reinitialize the field
            ...
            for x in test_board.fields:
                for f in x:
                    if f.figure_team != team:
                        AI_s.ai_helper_functions.mark_move_pattern(f,test_board)

            if test_board.fields[king_pos_y][king_pos_x].active == True:
                moves[i][j][1] = -1
            AI_s.ai_helper_functions.reset_markers(test_board)
            j += 1
        i += 1
